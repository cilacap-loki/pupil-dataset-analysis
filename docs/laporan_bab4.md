...
2.  **Dasar teori:** Deret data temporal biologis akan sangat jauh lebih mudah dianalisis secara mikroskopis oleh dokter apabila dipetakan pada sumbu kartesius dengan Waktu sebagai sumbu kontrol independen (Sumbu X) dan nilai pengukuran sebagai sumbu dependen (Sumbu Y).
...
1.  **Tujuan:** Mengekstraksi makna biologis tak beraturan (*Pupillary Hippus*) dari grafik *time-series* menjadi angka diagnostik mutlak yang pasti, yakni **Frekuensi (Hz)** dan **Fluktuasi Amplitudo (%)**.
...
1.  **Tujuan:** Mengukur dan memverifikasi seberapa akurat algoritma *V3 Engine (Machine Prediction)* dalam mendeteksi dan melacak letak koordinat sentroid pupil jika dibandingkan dan diadu dengan hasil anotasi kebenaran mutlak pakar manusia (*Ground Truth*).
2.  **Metrik evaluasi:** Menggunakan algoritma pengukur jarak standar internasional, **Mean Euclidean Error**. Algoritma ini menarik garis lurus terpendek antara titik potong (X, Y) tebakan program komputasi dengan titik (X, Y) referensi pakar pada potongan *frame* yang sama.
